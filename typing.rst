...
TypeScriptのベースになっているJavaScriptではな、長らくオブジェクトが配列かどうかを判定する明確な手法を提供してきませんでした。文字列にして、その結果をパースするとかも行われていました。ECMAScript 5の時代にようやく、 ``Array.isArray()`` というクラスメソッドが提供されるようになりました。
